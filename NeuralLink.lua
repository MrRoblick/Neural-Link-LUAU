...
local function genRandomIdx(N: number): {number}
	local A: {number} = table.create(N, 0)
	for i = 1, N do
		A[i] = i
	end
	for i = 1, N do
...
function nn.new(inputCount: number, hiddenCount: number, outputCount: number, regression: boolean, debugging: boolean?): NeuralNetwork
	...
	network.Rate1 = rate1
	network.Rate2 = rate2
	network.InputLayer = table.create(inputCount, 0)
	network.HiddenLayer = table.create(hiddenCount, 0)
	network.OutputLayer = table.create(outputCount, 0)
	
	network.ErrOutput = table.create(outputCount, 0)
	network.ErrHidden = table.create(hiddenCount, 0)
